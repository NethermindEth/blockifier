...
//            nonce.0,                                                    // Nonce.
        assert(tx_info.version == expected_tx_info.version, 'TX_INFO.VERSION_MISMATCH');
        assert(tx_info.account_contract_address == expected_tx_info.account_contract_address, 'TX_INFO.ACAM');
        assert(tx_info.max_fee == expected_tx_info.max_fee, 'TX_INFO.MAX_FEE_MISMATCH');
        assert(tx_info.signature == expected_tx_info.signature, 'TX_INFO.SIGNATURE_MISMATCH');
        assert(tx_info.transaction_hash == expected_tx_info.transaction_hash, 'TX_INFO.TX_HASH_MISMATCH');
        assert(tx_info.chain_id == expected_tx_info.chain_id, 'TX_INFO.CHAIN_ID_MISMATCH');
        assert(tx_info.nonce == expected_tx_info.nonce, 'TX_INFO.NONCE_MISMATCH');
