...
fn test_run_parallel_txs() {
    ...
    let thread_handle_1 = thread::spawn(move || {
        let result = account_tx_1.execute(&mut state_1, &block_context_1, true, true, None, false);
        assert_eq!(result.is_err(), enforce_fee);
    });

    let thread_handle_2 = thread::spawn(move || {
        account_tx_2.execute(&mut state_2, &block_context_2, true, true, None, false).unwrap();
