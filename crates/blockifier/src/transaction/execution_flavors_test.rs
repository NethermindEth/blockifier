...
fn test_simulate_validate_charge_fee_pre_validate(
    ...
    .execute(&mut state, &block_context, charge_fee, validate, None, None);
    assert_matches!(
        result.unwrap_err(),
...
fn test_simulate_validate_charge_fee_fail_validate(
    ...
    .execute(&mut falliable_state, &block_context, charge_fee, validate, None, None);
    if !validate {
...
fn test_simulate_validate_charge_fee_mid_execution(
    ...
    .execute(&mut state, &low_step_block_context, charge_fee, validate, None, None)
    .unwrap();
    assert!(tx_execution_info.revert_error.clone().unwrap().contains("no remaining steps"));
