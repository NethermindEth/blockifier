...
use super::sierra_utils::{
    ...
    encode_str_as_felts, felt_to_stark_felt, stark_felt_to_felt, u256_to_biguint,
};
...
impl<'state> StarkNetSyscallHandler for NativeSyscallHandler<'state> {
    ...
    fn get_execution_info_v2(
        ...
        let mut native_tx_info = TxV2Info {
            version: stark_felt_to_felt(tx_info.signed_version().0),
            ...
            signature: tx_info.signature().0.into_iter().map(stark_felt_to_felt).collect(),
            transaction_hash: stark_felt_to_felt(tx_info.transaction_hash().0),
            chain_id: Felt::from_hex(
                &self.execution_context.tx_context.block_context.chain_info.chain_id.as_hex(),
            )
            .unwrap(),
            nonce: stark_felt_to_felt(tx_info.nonce().0),
        ...
        let entry_point_selector = stark_felt_to_felt(self.entry_point_selector);
    ...
    fn deploy(
        ...
        let wrapper_calldata = Calldata(Arc::new(
            calldata.iter().map(|felt| felt_to_stark_felt(*felt)).collect::<Vec<StarkFelt>>(),
        ));
        ...
        let return_data = call_info.execution.retdata.0[..]
            .iter()
            .map(|felt| stark_felt_to_felt(*felt))
            .collect();
    ...
    fn replace_class(&mut self, class_hash: Felt, _remaining_gas: &mut u128) -> SyscallResult<()> {
        let class_hash = ClassHash(StarkHash::from(felt_to_stark_felt(class_hash)));
    ...
    fn library_call(
        ...
        let entry_point = CallEntryPoint {
            ...
            entry_point_selector: EntryPointSelector(StarkHash::from(felt_to_stark_felt(
                function_selector,
            ))),
    ...
    fn call_contract(
        ...
        let contract_address = ContractAddress::try_from(felt_to_stark_felt(address))
            .map_err(|error| encode_str_as_felts(&error.to_string()))?;
    ...
    fn storage_read(
        ...
        let key = StorageKey(
            PatriciaKey::try_from(felt_to_stark_felt(address))
                .map_err(|e| encode_str_as_felts(&e.to_string()))?,
        );
    ...
    fn storage_write(
        ...
        let write_result =
            self.state.set_storage_at(self.contract_address, key, felt_to_stark_felt(value));
    ...
    fn emit_event(
        ...
        let event = EventContent {
            keys: keys
                .iter()
                .map(|felt| EventKey(felt_to_stark_felt(*felt)))
                .collect::<Vec<EventKey>>(),
            data: EventData(data.iter().map(|felt| felt_to_stark_felt(*felt)).collect()),
        };
    ...
    fn send_message_to_l1(
        ...
        self.l2_to_l1_messages.push(OrderedL2ToL1Message {
            order,
            message: MessageToL1 {
                to_address: EthAddress::try_from(felt_to_stark_felt(to_address))
                    .map_err(|e| encode_str_as_felts(&e.to_string()))?,
                payload: L2ToL1Payload(
                    payload.iter().map(|felt| felt_to_stark_felt(*felt)).collect(),
                ),
            },
        });
