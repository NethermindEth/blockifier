...
fn tx_executor_test_body<S: StateReader>(
    ...
    let _tx_execution_info = tx_executor.execute(&tx, charge_fee, None, false).unwrap();
...
fn test_execute_txs_bouncing(block_context: BlockContext) {
    ...
    // Run.
    let results = tx_executor.execute_txs(&txs, true, None, false);
    ...
    let remaining_tx_results = tx_executor.execute_txs(remaining_txs, true, None, false);
